...
const UserBlogs = ({ blog, handelEdete }) => {
  ...
  return (
    ...
    >
      <Card
        style={{ width: "18rem" }}
        className="h-100 d-inline-block singel-blog"
      >
        <Card.Img variant="top" src={blog.image} />
        <Card.Body>
          <Card.Title>{blog.titel}</Card.Title>
          <Card.Text>
            <Link className="read-lenk" to={`/${blog._id}`}>
              {readMore
                ? blog.discripion
                : `${blog.discripion.substring(0, 100)}... `}{" "}
          ...
          <div className="btn">
            <TiEdit
              className="edite"
              id={blog._id}
              onClick={() => handelEdete(blog._id)}
            />
          </div>{" "}
          <div className="btn">
            <BsFillTrashFill
              className="delet"
              onClick={() => dispatch(deletBlog(blog._id))}
            />
          </div>
        </Card.Body>
      </Card>
