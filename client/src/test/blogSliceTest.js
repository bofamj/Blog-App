...
//*get a singel bloge
/* export const getSingelBloge = createAsyncThunk('/getSingelBlog',async (blogId,thunkAPI)=>{
    try {
        const token = thunkAPI.getState().user.user.token
        return await blogService.getSingelBloge(blogId,token)
    } catch (error) {
        const message = (error.response && error.response.message && error.response.data ) || error.message || error.toString()
         console.log(message);
    }
  ...
  extraReducers: (builder) => {
    builder
     ...
     .addCase(editeBlog.rejected,(state,action)=>{
      ...
      /* .addCase(getSingelBloge.pending,(state)=>{
        state.isLoading=true
    })
      .addCase(getSingelBloge.fulfilled,(state,action)=>{
            state.isError=false
            state.isSuccess=true
            state.isLoading=false
            state.blog = action.payload
     ...
     .addCase(getSingelBloge.rejected,(state,action)=>{
            state.isError=true
            state.isLoading=false
            state.massage=action.payload
        }) */
